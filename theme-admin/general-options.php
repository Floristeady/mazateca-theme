...
/*	1)	Add "theme Admin" link to left-nav Admin Menu & callback function for clicking that menu link */
	...
	if ( ! function_exists( 'create_theme_admin_page' ) ):
		function create_theme_admin_page() {
			add_theme_page( __('General Options', 'mazateca'), __( 'General Options', 'mazateca'), 'administrator', 'theme-admin', 'build_theme_admin_page');
		}
	...
	if ( ! function_exists( 'build_theme_admin_page' ) ):
		...
		?>
			<div id="theme-options-wrap">
				...
				<h2><?php _e( 'General options for', 'mazateca' ); ?> <?php echo wp_get_theme() . __( '', 'mazateca' );?> </h2>
				<p><?php _e( 'This is the web site manager. Select the options you want to include on the website.', 'mazateca' ); ?></p>
...
/*	3)	Add "theme Admin" Page options */
	...
	if ( ! function_exists( 'register_and_build_fields' ) ):
		function register_and_build_fields() {
			...
			//logo
			add_settings_field('logo_theme', __('Add Logotype:', 'mazateca'), 'logotype_setting', 'theme-admin', 'main_section');
	...
	if ( ! function_exists( 'logotype_setting' ) ):
		function logotype_setting() {
			...
			echo '<p><a href="'. $home .'/wp-admin/media-new.php" target="_blank"><em>'; _e( 'Upload your own logo image</em></a> using the WordPress Media Library and insert the URL here:</p>', 'mazateca' );
			echo '<input type="text" size="80" name="plugin_options[logo_theme_url]" value="'.$logo.'" onfocus="javascript:if(this.value===\'\'){this.select();}">';
			echo '<img style="margin-top: 10px; max-width:150px;" src="'. (($logo!=='') ? $logo :  get_template_directory_uri() .'/images/logo.png').'">';
		}
	endif; // logotype
	...
	if ( ! function_exists( 'google_verification_setting' ) ):
		function google_verification_setting() {
			...
			echo _e( '<p>Add <a href="https://support.google.com/webmasters/answer/35179">Google Verification</a> code and <a href="https://www.google.com/analytics">Google Universal Analytics </a> code to the <code>&lt;head&gt;</code> of all your pages.</p>', 'mazateca' );
			echo _e( '<p>To include Google Analytics and Google Verification code, select this option and add your Verification number here and the site domain. Google Analytics ID:</p>', 'mazateca' );
			...
			echo _e( '<p>This will add the following code to the <code>&lt;head&gt;</code> of your pages, where <code>XXXXXXXXX?</code> will be replaced with the code you insert above.</p><br/>', 'mazateca' );
			
			echo '<code>&lt;meta name="google-site-verification" content="'.$account.'"&gt;</code>';
			echo _e( '<p>And will add the latest version of Universal Analytics tracking code, to track website visitors with Google Analytics.</p>', 'mazateca' );
		}
	...
	if ( ! function_exists( 'meta_tags_setting' ) ):
		function meta_tags_setting() {
			...
			echo _e( '<p>Add meta keywords tags for SEO, write your keywords separated by a comma, but not a space.</p>', 'mazateca' );
			...
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages, where </code>tags...</code> will be replaced with the code you insert above</p>', 'mazateca' );
	...
	if ( ! function_exists( 'favicon_setting' ) ):
		function favicon_setting() {
			...
			echo _e( '<p>If you plan to use a <a href="http://en.wikipedia.org/wiki/Favicon">favicon</a> for your site, place the "favicon.ico" file in the root directory of your site.</p>', 'mazateca' );
			echo _e( '<p>If the file is in the right location, you don\'t really need to select this option, browsers will automatically look there and no additional code will be added to your pages.</p>', 'mazateca' );
			echo _e( '<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages:</p>', 'mazateca' );
	...
	if ( ! function_exists( 'modernizr_js_setting' ) ):
		function modernizr_js_setting() {
			...
			echo _e('<p><a href="http://modernizr.com">Modernizr</a> is a JS library that appends classes to the <code>&lt;html&gt;</code> that indicate whether the user\'s browser is capable of handling advanced CSS, like "cssreflections" or "no-cssreflections".  It\'s a really handy way to apply varying CSS techniques, depending on the user\'s browser\'s abilities, without resorting to CSS hacks.</p>', 'mazateca' );
			echo _e('<p>Selecting this option will add the following code to the <code>&lt;head&gt;</code> of your pages (note the lack of a version, when you\'re ready to upgrade, simply copy and paste the new version into the file below, and your site is ready to go!):</p>', 'mazateca' );
	...
	if ( ! function_exists( 'jquery_js_setting' ) ):
		function jquery_js_setting() {
			...
			echo _e('<p><a href="http://jquery.com/">jQuery</a> is a JS library that aids greatly in developing high-quality JavaScript quickly and efficiently.</p>', 'mazateca' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'mazateca' );
			...
			echo _e('<p><strong>Note: <a href="http://developer.yahoo.com/blogs/ydn/posts/2007/07/high_performanc_5/">Best-practices</a> recommend that you load JS as close to the <code>&lt;/body&gt;</code> as possible.  If for some reason you would prefer jQuery and jQuery plug-ins to be in the <code>&lt;head&gt;</code>, please select this option.</strong></p>', 'mazateca' );
			echo _e('<p>The above code first tries to download jQuery from Google\'s CDN (which might be available via the user\'s browser cache).  If this is not successful, it uses the theme\'s version.</p>', 'mazateca' );
			echo _e('<p><strong>Note: This plug-in tries to keep current with the most recent version of jQuery.  If for some reason you would prefer to use another version, please indicate that version:</strong></p><br/>', 'mazateca' );
	...
	if ( ! function_exists( 'site_js_setting' ) ):
		function site_js_setting() {
			...
			echo _e( '<p>If you would like to add your own site JavaScript file, we provides a starter file located in:</p>', 'mazateca' );
			echo '<code>' .BP_THEME_URL. '/js/script-starter.js</code>';
			echo _e( '<p>Add what you want to that file and select this option.</p>', 'mazateca' );
			echo  _e( '<p>Selecting this option will add the following code to your pages just before the: <code>&lt;/body&gt;</code></p>', 'mazateca' );
	...
	if ( ! function_exists( 'cache_buster_setting' ) ):
		function cache_buster_setting() {
			...
			echo _e( '<p>To force browsers to fetch a new version of a file, versus one it might already have cached, you can add a "cache buster" to the end of your CSS and JS files.</p>', 'mazateca' );
			echo _e( '<p>To increment the cache buster version number, type something here:</p><br />', 'mazateca' );
			echo '<input type="text" size="4" name="plugin_options[cache_buster_version]" value="'.$version.'"></p>';
			echo _e( '<p>Selecting this option will add the following code to the end of all of your CSS and JS file names on all of your pages:</p>', 'mazateca' );
